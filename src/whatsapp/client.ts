  ...
  function installReconnectionMonitor(sock: WASocket): void {
    sock.ev.on('connection.update', (update) => {
      ...
      if (connection === 'open') {
        ...
        cs.userLid = (sock.user as unknown as Record<string, unknown>)?.lid as string ?? null;
      ...
      if (connection === 'close') {
        ...
        if (code === DisconnectReason.loggedOut || code === 401) {
          rejectPending(new Error('WhatsApp session expired. Re-run `duo whatsapp-login`.'));
          return;
        }
        ...
        if (connection === 'close') {
            ...
            reconnect(authDir).then(() => {
              ...
              reconSock.ev.on('connection.update', (u) => {
                if (u.connection === 'open') {
                  ...
                  cs.userLid = (reconSock.user as unknown as Record<string, unknown>)?.lid as string ?? null;
                  cs.reconnectAttempts = 0;
                  cs.lastConnectedAt = Date.now();
          ...
          reject(new Error(
            code === DisconnectReason.loggedOut || code === 401
              ? 'WhatsApp session expired. Re-run `duo whatsapp-login`.'
